 ...
 */
public class OracleOptimizedFetchCursor implements IFetchCursor {
    ...
    private static final String SQL_TEMPLATE = "SELECT /*+ FIRST_ROWS(%s)*/  %s FROM %s WHERE %s = ? AND %s >= ?  AND %s <= ? ORDER BY %s, %s";
    private String sql;
    ...
    public OracleOptimizedFetchCursor(DBTool dbTool, long leftBorder, long rightBorder, long bucketId, TimerMapping mapping, ITimerHandleMapper handleMapper, long fetchSize) {
        ...
        sql = String.format(SQL_TEMPLATE, fetchSize,
                mapping.getListOfAll5Fields(),
                mapping.getTableName(),  mapping.getBucketIdField(), mapping.getExpireTimeField(),
                mapping.getExpireTimeField(),  mapping.getBucketIdField(), mapping.getExpireTimeField());
    }
