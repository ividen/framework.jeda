...
public class TestTransactionalMemoryQueueWithoutTrx extends TestEventQueue {
    ...
    public static TestSuite suite() {
        return new TestSuite(TestTransactionalMemoryQueueWithoutTrx.class);
    }

    public void testMaxSize() {
        TransactionalMemoryQueue memoryQueue = new TransactionalMemoryQueue(new NonTransactionalSystemManager());
        assertEquals("MaxSize wrong", Long.MAX_VALUE, memoryQueue.getMaxSize());
    }

    @Override
    protected IQueue createQueue() {
        return new TransactionalMemoryQueue(new NonTransactionalSystemManager(), ObjectCloneType.SERIALIZE, 10l);
    }
}